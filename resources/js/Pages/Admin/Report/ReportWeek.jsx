...
import '../../../../scss/admin/report/_reportweek.scss';

import AdminLayout from "@/Layouts/AdminLayout";
import DashboardLayout from '../DashboardLayout';
import { formatedCurrency, limitString } from "@/Utils/String";
import { convertWeekReadble, endDateFormatted, startDateFormatted, weekYW } from "@/Utils/Date";

export default function ReportWeek({notes}) {
    ...
    const hitungCostTotal = notes?.filter(e => {
        const createdDate = moment(e.created_transaction_at.slice(0, 10), 'YYYY-MM-DD');
    ...
    return (
        <div className="report-week-component">
            <div id="container_report_week">
                <Breadcrumbs className='mt-5 bg-gray-200'>
                    ...
                    <Link href="/sales-note/report-period/week" className='montserrat font-medium'>Periode Mingguan</Link>
                </Breadcrumbs>
                <div className="date-download-section flex 3xs:flex-col gap-5 items-center justify-between mt-10">
                    ...
                    <input type="week" id="date" name='datekey' value={date} onChange={handleWeekChange} className='montserrat rounded-[5px] 3xs:w-full w-[20rem] bg-blue-600 border-0 text-white' />
                </div>
                {
                    dialogDownloadFile &&
                        ...
                        <thead className="text-white uppercase poppins">
                            <tr className="bg-transparent border-x border-t border-b-0 border-orange-600">
                                <th scope="col" colSpan={9} className="px-6 py-6 text-neutral-900 text-[1.25rem]">
                                    Penjualan Barang - {convertWeekReadble(date)}
                                </th>
                            </tr>
