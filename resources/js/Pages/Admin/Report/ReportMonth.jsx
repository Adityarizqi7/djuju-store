...
import '../../../../scss/admin/report/_reportmonth.scss';
...
export default function ReportMonth({notes, modal}) {
    ...
    const hitungModalTotal = modal?.filter((value) => {
        if (date) {
            if (value?.asset_time === date) return true;
            return false;
        }
        return true;
    })
    ?.reduce((accumulator, currentValue) => {
        return accumulator + currentValue.initial_price;
    }, 0);

    const costTotal = formatedCurrency(hitungCostTotal)
    const profitTotal = formatedCurrency(hitungCostTotal - hitungModalTotal)
    ...
    const toPdf = () => {
        ...
        var text = doc.splitTextToSize(`Laporan Catatan Penjualan Pada JujuMart - ${convertMonthReadble(date)}`, pageWidth - 30, {})
        ...
        <div className="report-month-component">
            <div id="container_report_month">
                <Breadcrumbs className='mt-5 bg-gray-200'>
                    ...
                    <Link href="/sales-note/report-period/month" className='montserrat font-medium'>Periode Bulanan</Link>
                </Breadcrumbs>
                <div className="date-download-section flex 3xs:flex-col gap-5 items-center justify-between mt-10">
                    ...
                    <input type="month" id="date" name='datekey' value={date} onChange={handleMonthChange} className='montserrat rounded-[5px] 3xs:w-full w-[20rem] bg-blue-600 border-0 text-white' />
                </div>
                {
                    dialogDownloadFile &&
                        ...
                        <li className='hover:bg-gray-200/50'>
                         <button type='button' className='w-full transition-colors duration-200 text-green-600 px-2 py-4 text-[1rem] montserrat flex items-center gap-1 font-semibold'>
                                ...
                                <ReactHtmlTableToExcel3 
                                    ...
                                    table="table-data"
                                    filename={`Laporan Penjualan Pada JujuMart - ${convertMonthReadble(date)}`}
                                    filetype="xls"
                                    sheet="tablexls"
                                    buttonText="Excel"
                                />
                        ...
                        <thead className="text-white uppercase poppins">
                            <tr className="bg-transparent border-x border-t border-b-0 border-orange-600">
                                <th scope="col" colSpan={9} className="px-6 py-6 text-neutral-900 text-[1.25rem]">
                                    Penjualan Barang - {convertMonthReadble(date)}
                                </th>
                            </tr>
...
ReportMonth.layout = page => (
    <AdminLayout title='Laporan Bulanan - Admin JujuMart' keyword='laporan bulanan catatan penjualan barang dagangan JujuMart' desc='Halaman untuk megelola laporan mingguan dagangan yang ada pada JujuMart' >
